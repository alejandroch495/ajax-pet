...
if (viewRenderedSource) {//if viewRenderedSource var is true (above) we'll initialize initRendered() which will add a button to view rendered source
 ...
 * @todo none
 */
function addOnload(myfunc) {//addOnload allows us to attach multiple functions to the page load event
 ...
 * @todo none
 */
function checkBrowser(str) {//quickly separate a browser from the herd with an identifying userAgent string
    foundBrowser = false;
...
function viewRendered() {
    ...
    try {//does window already exist?
        myWindow.document.write(rendered);
        myWindow.document.bgColor = "#" + color;
        myWindow.focus();
        myWindow.document.close();
    } catch (ex) {//no window, IE misbehaviors, etc.
 ...
 * @todo none
 */
function empty(fObj, msg) {//will accept form elements and check for input

    //radio buttons & checkboxes are arrays - their type must be determined by a single element
    if (fObj.length != undefined) {//if length is defined, more than one element. Treat as an array
    ...
    switch (testType) {//once the form object type is determined, we can treat elements separately 

        //handle radio & checkbox elements
        case "radio":
        case "checkbox":
            if (isArray) {//if length is defined, more than one element. Treat as an array
 ...
 * @todo none
 */
function isEmail(fObj, msg) {//Uses regular expression for email check
 ...
 * @todo none
 */
function isAlpha(fObj, msg) {//Uses regular expression for email check
 ...
 * @todo none
 */
function isAlphanumeric(fObj, msg) {//Uses regular expression for alphabetic check
 ...
 * @todo none
 */
function correctLength(fObj, min, max, msg) {//Uses regular expression for email check
    var rePattern = /^[a-zA-Z]+$/;
